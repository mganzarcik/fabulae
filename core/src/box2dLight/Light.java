...
public abstract class Light {
	...
	public static final Color DefaultColor = new Color(0.75f, 0.75f, 0.5f, 0.75f);
	...
	protected RayCastCallback raycastCallback = null;
	...
	public void setColor(Color newColor) {
		if (newColor != null) {
			if (CoreUtil.equals(color, newColor)) {
				return;
			}
			color.set(newColor);
	...
	abstract protected void setMesh();
	...
	/**
	 * Marks the light as dirty, forcing it to properly recalculate 
	 * relevant attributes next time it is updated.
	 */
	...
	private final void setRayNum(int rays) {
		...
		nonSoftSegments = new float[vertexNum * 8];
		softSegments = new float[vertexNum * 8];
	...
	protected RayCastCallback getRayCastCallback() {
		if (raycastCallback == null) {
			raycastCallback = new RayCastCallback() {
				@Override
				final public float reportRayFixture(Fixture fixture,
