...
public class DirectionalLight extends Light {
	...
	protected void update () {
		...
		for (int i = 0; i < rayNum; i++) {
			...
			if (rayHandler.world != null && !xray) {
				rayHandler.world.rayCast(getRayCastCallback(), start[i], end[i]);
			}
		}
		setMesh();
	}
	...
	protected void setMesh() {
		...
		for (int i = 0; i < rayNum; i++) {
			nonSoftSegments[nonSoftIndex++] = start[i].x;
			nonSoftSegments[nonSoftIndex++] = start[i].y;
			nonSoftSegments[nonSoftIndex++] = colorF;
			nonSoftSegments[nonSoftIndex++] = 1f;
			...
			if (soft && !xray) {
				...
				softSegments[softIndex++] = 1f;
	
				softSegments[softIndex++] = mx[i] + softShadowLenght * cos;
				softSegments[softIndex++] = my[i] + softShadowLenght * sin;
